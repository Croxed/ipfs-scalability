...
for NODES in "${CLUSTER_NODES[@]}"; do
	...
	for cluster in "${array[@]}" ; do
		ssh -n -f root@"$cluster" bash -c "'(pkill ipfs; pkill ipfs; pkill bash; cd /root/ipfs-scalability; nohup bash /root/ipfs-scalability/deploy_cluster.sh $NODE_0_ADDR $NODES > /root/ipfs-scalability/daemon.out 2>&1) &'"
	done
